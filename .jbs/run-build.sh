...
export PROJECT_VERSION=33.1.0-jre
...
if [ -n "" ]; then
  echo "Setting version to 33.1.0-jre to match enforced version"
  mvn -B -e -s "/root/software/settings/settings.xml" -t "/root/software/settings/toolchains.xml" org.codehaus.mojo:versions-maven-plugin:2.8.1:set -DnewVersion="33.1.0-jre" | tee /root/project/logs/enforce-version.log
fi
